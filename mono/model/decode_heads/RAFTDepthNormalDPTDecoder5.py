...
class Roughness_head(nn.Module):
    ...
    def __init__(self, input_channels=96):
        ...
        self.conv_blk3=ConvBlock(channels=64)
        self.conv_blk4=ConvBlock(channels=64)
    ...
    def forward(self, x):
        return self.head(self.conv_blk4(self.conv_blk3(self.conv_blk2(self.conv_blk1(self.channel_fix(x))))))

class Metallic_head(nn.Module):
    def __init__(self, input_channels=96):
        super(Metallic_head, self).__init__()
        self.channel_fix = nn.Conv2d(input_channels,64, kernel_size=3, stride=1, padding=1)
        self.conv_blk1=ConvBlock(channels=64)
        self.conv_blk2=ConvBlock(channels=64)
        self.conv_blk3=ConvBlock(channels=64)
...
class RAFTDepthNormalDPT5(nn.Module):
    def __init__(self, cfg):
        ...
        # self.metallic_head=Metallic_head(input_channels=96)
        self.relu = nn.ReLU(inplace=True)
        self.pratham=None
    ...
    def forward(self, vit_features,gray_images, **kwargs):
        ...
        # metallic_pred=self.metallic_head(feature_map)
